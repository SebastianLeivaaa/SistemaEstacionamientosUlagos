...
export const SearchByPatent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            try {
                const response = await axios.post("/api/get-record-reservation-by-patente", formData, { withCredentials: true });
                console.log(response.data)
                navigate('/record-reservation-by-patente', { state: { recordReservation: response.data } } )
            }catch(error){
    ...
    return (
        <div className="flex flex-col w-full gap-y-8">
            <div className="flex flex-col gap-y-4 max-md:gap-y-2">
                <label className="text-xl max-md:text-lg">Patente</label>
                <input
                    type='text'
                    id='vehiclePatente'
                ...
                {errors.vehiclePatente && (
                    <p className='text-red-600 text-base max-md:text-center'>{errors.vehiclePatente}</p>
                )}
            </div>
            <div className="flex flex-col gap-y-4 max-md:gap-y-2">
                <label className="text-xl max-md:text-lg">Fecha (opcional)</label>
                <input
                    type='date'
                    id='date'
                    onChange={handleChange}
