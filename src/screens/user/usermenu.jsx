...
export const Usermenu = () => {
    ...
    const getProfile = async () => {
        try {
            const response = await axios.get("http://localhost:3090/api/profile");
    ...
    // Usa useEffect para llamar a getProfile cuando el componente se monta
    useEffect(() => {
        getProfile();
    }, []);
            ...
            <div className="flex flex-col items-center p-8 gap-y-8 rounded-md max-md:w-[75%] max-md:px-4 max-md:py-8 bg-white-50">
                <div className="flex flex-row w-full justify-between">
                    ...
                    <div className="flex flex-col items-start">
                        <h1 className="text-xl font-bold text-center max-md:text-base text-congress-blue-900">{JSON.stringify(user,null,2)}</h1>
                        <button onClick={getProfile} className="mt-4 w-full bg-white font-bold text-red-600  text-lg  flex flex-row items-center justify-center gap-x-1 max-md:text-base"><HiOutlineLogin className="text-3xl max-md:text-2xl"/>CERRAR SESIÓN</button>
                    </div>
                </div>
