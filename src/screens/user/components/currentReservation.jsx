...
const deleteReservation = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post('http://localhost:3090/api/delete-reservation', { userRut: user.userRut }, { withCredentials: true });
            ...
            <div className="dark:bg-midnight-950 bg-white-50 shadow-3xl  contrast-[95%] rounded-md p-8 max-md:p-4 w-full flex flex-col gap-y-8 max-md:gap-y-4">
                ...
                <div className="flex flex-row gap-x-8 w-full justify-start max-lg:grid max-lg:grid-cols-4 max-lg:gap-x-2 max-sm:gap-x-2 gap-y-3">
                    {currentReservation.length > 0 ? (
                        <>
                            <div className="flex flex-col p-2 bg-midnight-800 rounded-md w-full h-full items-center">
                            ...
                            <button className="flex flex-col p-2 bg-green-700 hover:bg-green-800 rounded-md w-full h-full max-lg:col-span-2 max-sm:h-auto items-center justify-center" onClick={openModal}>
                                <span className="text-white-50 font-bold text-center text-xl max-md:text-base max-sm:text-xs">Ver tu código QR</span>
                            </button>
                            <button className="flex flex-col p-2 bg-red-500 hover:bg-red-600 rounded-md w-full h-full max-lg:col-span-2 max-sm:h-auto items-center justify-center" onClick={confirmDelete}>
